...
def _norm_sim_col_name(col: object) -> str:
    """Normaliza nombres de columnas de la tabla de simulación inicial."""
...
def _read_initial_simulation_text(text: str) -> pd.DataFrame:
    """Lee una tabla pegada desde Excel/Teams con varias filas de encabezado."""
    ...
    for i, ln in enumerate(lines):
        low = _norm_sim_col_name(ln)
        if low.startswith("fecha") and ("elev" in low or "alha" in low or "gat" in low):
            header_idx = i
            break
    if header_idx is None:
        # Si solo viene una tabla simple con el header en la primera fila.
        header_idx = 0

    body = "\n".join(lines[header_idx:])
    # Preferencia: tabla copiada desde Excel usa tabulaciones. Respaldo: 2+ espacios.
    try:
        if "\t" in body:
            df = pd.read_csv(StringIO(body), sep="\t", engine="python")
        else:
            df = pd.read_csv(StringIO(body), sep=r"\s{2,}", engine="python")
    except Exception:
        try:
            df = pd.read_csv(StringIO(body), sep=",", engine="python")
        except Exception:
...
def _read_initial_simulation_file(file_bytes: bytes, filename: str = "") -> pd.DataFrame:
    """Lee una tabla de simulación inicial desde Excel/CSV/TXT."""
    if not file_bytes:
        return pd.DataFrame()
    ...
    if name.endswith((".xlsx", ".xlsm")):
        ...
        for i in range(min(len(raw), 20)):
            ...
            low = _norm_sim_col_name(row_txt)
            if "fecha" in low and ("elev" in low or "alha" in low or "gat" in low):
                header_idx = i
                break
    ...
    try:
        ...
        text = file_bytes.decode("latin-1", errors="replace")
    return _read_initial_simulation_text(text)


def parse_initial_simulation_table(
    pasted_text: str = "",
    file_bytes: Optional[bytes] = None,
    filename: str = "",
) -> pd.DataFrame:
    """Convierte la tabla inicial a columnas canónicas.

    Unidades esperadas en la entrada:
    - Niveles: pies.
    - Aportes, vertidos y esclusajes: m³/s.
    - Hidrogeneración: MW.
    """
    df = _read_initial_simulation_file(file_bytes, filename) if file_bytes else _read_initial_simulation_text(pasted_text)
    if df is None or df.empty:
        return pd.DataFrame()
    ...
    used = set()
    for c in df.columns:
        n = _norm_sim_col_name(c)
        target = None
        if "fecha" in n or n in {"date", "datetime", "sello"}:
            target = "Fecha"
        elif "elev" in n and ("alha" in n or "mad" in n):
            target = "NP_ALH_FT"
        elif "elev" in n and "gat" in n:
            target = "NP_GAT_FT"
        elif ("at" in n or "aporte" in n or "ap" in n) and ("alha" in n or "mad" in n):
            target = "AP_ALH_M3S"
        elif ("at" in n or "aporte" in n or "ap" in n) and "gat" in n:
            target = "AP_GAT_M3S"
        elif "hidro" in n and ("alha" in n or "mad" in n):
            target = "HP_ALH_MW"
        elif "hidro" in n and "gat" in n:
            target = "HP_GAT_MW"
        elif "spill" in n and ("mad" in n or "alha" in n):
            target = "SPILL_ALH_M3S"
        elif "spill" in n and "gat" in n:
            target = "SPILL_GAT_M3S"
        elif "vert" in n and ("mad" in n or "alha" in n):
            target = "SPILL_ALH_M3S"
        elif "vert" in n and "gat" in n:
            target = "SPILL_GAT_M3S"
        elif "esclus" in n:
            target = "ESCL_GAT_M3S"
    ...
    if out.empty:
        return pd.DataFrame()
    out["Fecha_dia"] = out["Fecha"].dt.normalize()

    numeric_cols = [c for c in out.columns if c not in ("Fecha", "Fecha_dia")]
    for c in numeric_cols:
        out[c] = pd.to_numeric(out[c].astype(str).str.replace(",", ".", regex=False), errors="coerce")

    keep = ["Fecha", "Fecha_dia"] + [
        c for c in [
            "NP_ALH_FT", "NP_GAT_FT", "AP_ALH_M3S", "AP_GAT_M3S",
            "HP_ALH_MW", "HP_GAT_MW", "SPILL_ALH_M3S", "SPILL_GAT_M3S", "ESCL_GAT_M3S",
        ] if c in out.columns
    ]
    return out[keep].sort_values("Fecha_dia")


def _set_export_col_value(df: pd.DataFrame, mask, includes: List[str], create_col: str, value) -> None:
    """Asigna valor a la primera columna que contenga todos los textos; si no existe, la crea."""
    def ok(col: str) -> bool:
        low = str(col).lower()
        return all(str(x).lower() in low for x in includes)
    matches = [c for c in df.columns if ok(c)]
    col = matches[0] if matches else create_col
    if col not in df.columns:
        df[col] = np.nan
    df.loc[mask, col] = value


def apply_initial_simulation_to_export_table(
    table: pd.DataFrame,
    sim_df: pd.DataFrame,
    res_key: str,
    pct_ref: int,
    ...
    """Integra una simulación inicial a los primeros días de la tabla exportable.

    Solo afecta la tabla de exportación; no modifica los datos DSS internos ni las demás pestañas.
    Las fechas de la simulación que coinciden con el DSS reemplazan esos días. Si una fecha no existe
    en el rango exportado, se agrega como fila nueva y se ordena cronológicamente.
    """
    if table is None or table.empty or sim_df is None or sim_df.empty:
        return table

    out = table.copy()
    out["Fecha"] = pd.to_datetime(out["Fecha"], errors="coerce")
    out = out[out["Fecha"].notna()].copy()
    out["_fecha_key"] = out["Fecha"].dt.normalize()

    sim = sim_df.copy()
    sim["Fecha_dia"] = pd.to_datetime(sim["Fecha_dia"], errors="coerce").dt.normalize()
    sim = sim[sim["Fecha_dia"].notna()].sort_values("Fecha_dia")
    if sim.empty:
        out.drop(columns=["_fecha_key"], inplace=True, errors="ignore")
        return out

    cfg_name = "Gatún" if res_key == "gatun" else "Alhajuela / Madden"
    source_col = "Fuente / ajuste"
    ...
    for _, row in sim.iterrows():
        fkey = row["Fecha_dia"]
        mask = out["_fecha_key"] == fkey
        if not bool(mask.any()):
            new_row = {c: np.nan for c in out.columns}
            new_row["Fecha"] = fkey
            new_row["_fecha_key"] = fkey
            new_row["Embalse"] = cfg_name
            new_row["Percentil solicitado"] = f"P{int(pct_ref)}"
            new_row["Percentiles usados"] = f"Simulación inicial integrada como P{int(pct_ref)}"
            new_row[source_col] = "Simulación inicial"
            out = pd.concat([out, pd.DataFrame([new_row])], ignore_index=True)
            mask = out["_fecha_key"] == fkey

        out.loc[mask, source_col] = "Simulación inicial"

        np_col = col_map.get("np")
        if np_col and np_col in row.index and pd.notna(row[np_col]):
            _set_export_col_value(out, mask, ["NP", "ft"], f"NP P{int(pct_ref)} (ft PLD)", float(row[np_col]))

        hp_col = col_map.get("hp")
        if hp_col and hp_col in row.index and pd.notna(row[hp_col]):
            _set_export_col_value(out, mask, ["HP", "MW"], f"HP P{int(pct_ref)} (MW)", float(row[hp_col]))

        ap_col = col_map.get("ap")
        if ap_col and ap_col in row.index and pd.notna(row[ap_col]):
            ap_m3s = float(row[ap_col])
            ap_cfs = ap_m3s / CFS_TO_M3S
            ap_neto_cfs = max(ap_cfs - clean_evap_cfs(evap_cfs), 0.0)
            _set_export_col_value(out, mask, ["AP neto", "p³/s"], f"AP neto DSS P{int(pct_ref)} (p³/s)", ap_neto_cfs)
            _set_export_col_value(out, mask, ["Evaporación sumada"], "Evaporación sumada AP DSS (p³/s)", clean_evap_cfs(evap_cfs))
            _set_export_col_value(out, mask, ["AP total", "p³/s"], f"AP total DSS P{int(pct_ref)} (p³/s)", ap_cfs)
            _set_export_col_value(out, mask, ["AP total", unit_label(flow_unit)], f"AP total DSS P{int(pct_ref)} ({unit_label(flow_unit)})", convert_flow(pd.Series([ap_cfs]), flow_unit).iloc[0])
            _set_export_col_value(out, mask, ["AP sim", "m³/s"], f"AP simulación inicial P{int(pct_ref)} (m³/s)", ap_m3s)

        spill_col = col_map.get("spill")
        if spill_col and spill_col in row.index and pd.notna(row[spill_col]):
            spill_m3s = float(row[spill_col])
            spill_cfs = spill_m3s / CFS_TO_M3S
            _set_export_col_value(out, mask, ["Vertido", "p³/s"], f"Vertido DSS P{int(pct_ref)} (p³/s)", spill_cfs)
            _set_export_col_value(out, mask, ["Vertido", unit_label(flow_unit)], f"Vertido DSS P{int(pct_ref)} ({unit_label(flow_unit)})", convert_flow(pd.Series([spill_cfs]), flow_unit).iloc[0])
            _set_export_col_value(out, mask, ["Vertido sim", "m³/s"], f"Vertido simulación inicial P{int(pct_ref)} (m³/s)", spill_m3s)

        escl_col = col_map.get("escl")
        if escl_col and escl_col in row.index and pd.notna(row[escl_col]):
            escl_m3s = float(row[escl_col])
            escl_cfs = escl_m3s / CFS_TO_M3S
            _set_export_col_value(out, mask, ["Total esclusajes"], f"Total esclusajes EG+EP ({unit_label(flow_unit)})", convert_flow(pd.Series([escl_cfs]), flow_unit).iloc[0])
            _set_export_col_value(out, mask, ["Esclusajes sim", "m³/s"], f"Esclusajes simulación inicial P{int(pct_ref)} (m³/s)", escl_m3s)

    out.drop(columns=["_fecha_key"], inplace=True, errors="ignore")
    for c in out.columns:
        if c not in ("Fecha", "Embalse", "Percentil solicitado", "Percentiles usados", source_col):
            out[c] = pd.to_numeric(out[c], errors="coerce").round(3)
...
def tab_exportar_percentil(
    ...
    # Opción quirúrgica: integrar una simulación inicial solo para la exportación.
    # La tabla pegada/cargada puede contener ambos embalses; se usa únicamente el embalse seleccionado.
    with st.expander("➕ Integrar simulación inicial a los primeros días", expanded=False):
        st.caption(
            "Pegue la tabla desde Excel o cargue un CSV/XLSX con columnas: Fecha, Elev. Alha, Elev. Gat, "
            "AT. Alha, AT. Gat, Hidro Alha, Hidro Gat, Spill Mad, Spill Gat y Esclusajes. "
            "Los aportes, vertidos y esclusajes deben estar en m³/s. La integración se aplica al P seleccionado."
        )
        aplicar_sim = st.checkbox(
            "Aplicar simulación inicial en la tabla exportable",
            value=False,
            key="exp_apply_initial_sim",
        )
        sim_file = st.file_uploader(
            "Cargar tabla de simulación inicial (CSV, TXT o XLSX)",
            type=["csv", "txt", "xlsx", "xlsm"],
            key="exp_initial_sim_file",
        )
        sim_text = st.text_area(
            "O pegar tabla aquí",
            value="",
            height=180,
            key="exp_initial_sim_text",
            placeholder="Pegue aquí la tabla copiada desde Excel, incluyendo la fila: fecha | Elev. Alha | Elev. Gat | AT. Alha | AT. Gat | ...",
        )
        sim_df = pd.DataFrame()
        if aplicar_sim:
            if sim_file is not None:
                sim_df = parse_initial_simulation_table(
                    file_bytes=sim_file.getvalue(),
                    filename=sim_file.name,
                )
            elif sim_text.strip():
                sim_df = parse_initial_simulation_table(pasted_text=sim_text)

            if sim_df.empty:
                st.info("Active la opción y cargue o pegue una tabla válida para integrarla.")
            else:
                st.success(f"Simulación inicial reconocida: {len(sim_df):,} filas. Se integrará al embalse {embalse_op} como P{int(pct)}.")
                st.dataframe(sim_df.drop(columns=["Fecha_dia"], errors="ignore"), use_container_width=True, hide_index=True, height=220)
                table = apply_initial_simulation_to_export_table(
                    table,
                    sim_df,
                    res_key=res_key,
                    pct_ref=int(pct),
