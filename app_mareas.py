...
def detectar_unidad_csv(fuente):
    ...
    metros o pies. Busca 'Value (m)' o 'Value (ft)' en las primeras 6 filas del CSV.
...
def cargar_csv(fuente, unidad_forzada=None):
    ...
    # Leer datos
    df = pd.read_csv(fuente, skiprows=5, names=["fecha", "valor_raw"])
...
def candidate_data_directories():
    roots = [
        Path(_dir),
        Path(_dir) / "data",
        Path(_dir) / "fuentes",
...
def calcular_tendencia_nivel(df_in, col, base="mensual", min_puntos=3, filtrar_outliers=False):
    base = base.lower().strip()
    serie_src = df_in[["fecha", col]].dropna().copy().sort_values("fecha")
    if serie_src.empty:
        return None

    if base == "anual":
        serie = serie_src.set_index("fecha")[col].resample("YE").mean().dropna()
        x_vals = serie.index.year.astype(float)
        etiqueta_serie = "Nivel medio anual observado"
        etiqueta_y = f"Nivel medio anual ({'m' if col.endswith('_m') else 'ft'})"
        texto_base = "anuales"
    else:
        serie = serie_src.set_index("fecha")[col].resample("MS").mean().dropna()
        x_vals = serie.index.year + (serie.index.dayofyear - 1) / 365.25
        etiqueta_serie = "Nivel medio mensual observado"
        etiqueta_y = f"Nivel medio mensual ({'m' if col.endswith('_m') else 'ft'})"
        texto_base = "mensuales"

    if len(serie) < min_puntos:
        return None

    serie_original = serie.copy()
    n_outliers = 0
    if filtrar_outliers and len(serie) >= max(8, min_puntos):
        med = np.median(serie.values)
        mad = np.median(np.abs(serie.values - med))
        if mad > 0:
            robust_z = 0.6745 * (serie.values - med) / mad
            mask = np.abs(robust_z) <= 3.5
            n_outliers = int((~mask).sum())
            serie = serie[mask]
            x_vals = np.asarray(x_vals)[mask]

    if len(serie) < min_puntos:
        return None

    x_vals = np.asarray(x_vals, dtype=float)
    y_vals = np.asarray(serie.values, dtype=float)
    slope, intercept, r_val, p_val, std_err = sp_stats.linregress(x_vals, y_vals)
    trend_line = intercept + slope * x_vals

    n = len(y_vals)
    resid = y_vals - trend_line
    dof = max(n - 2, 1)
    s_err = np.sqrt(np.sum(resid**2) / dof) if n > 2 else np.nan
    tcrit = sp_stats.t.ppf(0.975, dof) if n > 2 else np.nan
    ci_low = slope - tcrit * std_err if np.isfinite(tcrit) else np.nan
    ci_high = slope + tcrit * std_err if np.isfinite(tcrit) else np.nan

    unidad_base = "m" if col.endswith("_m") else "ft"
    factor_mm = 1000 if unidad_base == "m" else FT_TO_M * 1000
    slope_mm_year = slope * factor_mm
    ci_low_mm_year = ci_low * factor_mm if pd.notna(ci_low) else np.nan
    ci_high_mm_year = ci_high * factor_mm if pd.notna(ci_high) else np.nan

    alertas = []
    if base == "mensual" and n < 24:
        alertas.append("Serie mensual corta: se recomiendan al menos 24 meses para una evaluación más sólida.")
    if base == "anual" and n < 5:
        alertas.append("Serie anual corta: se recomiendan al menos 5 años para una evaluación más sólida.")
    if p_val >= 0.05:
        alertas.append("La pendiente no es estadísticamente significativa al 95%.")
    if filtrar_outliers and n_outliers > 0:
        alertas.append(f"Se excluyeron {n_outliers} valores agregados atípicos mediante MAD robusto.")
    ...
    return {
        ...
        "s_err": float(s_err) if np.isfinite(s_err) else np.nan,
        "slope_mm_year": float(slope_mm_year),
        "ci_low": float(ci_low) if pd.notna(ci_low) else np.nan,
        "ci_high": float(ci_high) if pd.notna(ci_high) else np.nan,
        "ci_low_mm_year": float(ci_low_mm_year) if pd.notna(ci_low_mm_year) else np.nan,
        "ci_high_mm_year": float(ci_high_mm_year) if pd.notna(ci_high_mm_year) else np.nan,
        "etiqueta_serie": etiqueta_serie,
        "etiqueta_y": etiqueta_y,
        "texto_base": texto_base,
        "periodo_ini": int(serie.index.min().year),
        "periodo_fin": int(serie.index.max().year),
        "n_total_agregado": int(len(serie_original)),
        "n_utilizado": int(len(serie)),
...
# Estaciones preferidas por puente (en orden de preferencia).
# Pacífico: AMA primero, DHT como alternativa.
# Atlántico: LMB.
...
if not datasets:
    st.markdown(
        "<h1 style='color:#1a5276; text-align:center; margin-top:60px;'>"
    ...
    st.markdown("---")
    st.markdown(
        "<h3 style='text-align:center; color:#1a5276;'>📋 Guía de Uso Simple — Paso a Paso</h3>",
        unsafe_allow_html=True,
    )

    pasos = [
        ("1️⃣", "Obtén tus datos",
         "Exporta los datos de marea desde el sistema ACP en formato CSV. "
         "El archivo debe llamarse <b>BulkExport-LMB-*.csv</b> (Atlántico), "
         "<b>BulkExport-DHT-*.csv</b> (Pacífico — Diablo Heights) o "
         "<b>BulkExport-AMA-*.csv</b> (Pacífico — Amador). "
         "El dashboard detecta automáticamente la estación por el nombre del archivo."),
        ("2️⃣", "Carga el archivo — dos opciones",
         "<b>Opción A · Subir archivo(s):</b> En la barra lateral, selecciona "
         "<b>«Subir archivo(s)»</b> y arrastra uno o más CSV desde tu equipo. "
         "Útil cuando trabajas con datos recién exportados o en cualquier máquina.<br><br>"
         "<b>Opción B · Archivos locales:</b> Selecciona <b>«Archivos locales»</b> para "
         "que el dashboard detecte automáticamente los archivos "
         "<code>BulkExport-*.csv</code> que estén en la misma carpeta del script. "
         "Ideal si ejecutas la app localmente y mantienes los CSV junto al archivo "
         "<code>app_mareas.py</code>. Puedes activar/desactivar cada estación con el "
         "checkbox correspondiente. En ambos modos puedes cargar las tres estaciones "
         "simultáneamente para habilitar el análisis comparativo."),
        ("3️⃣", "Verifica las unidades",
         "El dashboard <b>auto-detecta</b> si los datos están en metros (m) o pies (ft) "
         "leyendo la cabecera del CSV. Si lo necesitas, puedes forzar la unidad manualmente "
         "en la sección <b>«Unidades de Entrada»</b> de la barra lateral."),
        ("4️⃣", "Define el rango de fechas",
         "El rango por defecto inicia el <b>01/01/2004</b> hasta el último dato disponible. "
         "Ajusta las fechas de inicio y fin en la barra lateral para enfocarte en un período "
         "específico de análisis."),
        ("5️⃣", "Selecciona la unidad de visualización",
         "Elige entre <b>Pies (ft)</b> o <b>Metros (m)</b> para todos los gráficos y tablas. "
         "El cambio se aplica instantáneamente en toda la aplicación."),
        ("6️⃣", "Explora las pestañas de análisis",
         "Navega por las pestañas superiores: <b>Resumen</b> (últimos 7 días), "
         "<b>Planos de Referencia</b> (HAT, MHHW, MHW, MSL, MLW…), "
         "<b>Predicción Astronómica</b>, <b>Serie Temporal</b>, <b>Tendencia</b>, "
         "<b>STL</b>, <b>Heatmap</b> y más."),
        ("7️⃣", "Predicción astronómica (opcional)",
         "Sube el archivo Excel de <b>constituyentes armónicas</b> "
         "(<code>Constituents_limon_2025.xlsx</code>) en la sección "
         "<b>«Constituyentes Armónicas»</b> de la barra lateral para habilitar la pestaña "
         "de predicción con correcciones nodales de Schureman (1958)."),
        ("8️⃣", "Verifica el paso bajo puentes",
         "Ve a la pestaña <b>🚢 Paso bajo puentes</b> para calcular el margen de altura "
         "libre disponible para cualquier buque en los puentes del Canal "
         "(Américas, Atlántico, Centenario), usando el nivel de marea actual o manual."),
        ("9️⃣", "Exporta los resultados",
         "En la pestaña <b>📥 Exportar</b> descarga en CSV los datos filtrados, "
         "los eventos de pleamar/bajamar detectados y los planos de referencia calculados."),
    ]

    for emoji, titulo, descripcion in pasos:
        with st.container():
            col_ic, col_txt = st.columns([0.08, 0.92])
            with col_ic:
                st.markdown(
                    f"<div style='font-size:2rem; text-align:center; padding-top:6px;'>{emoji}</div>",
                    unsafe_allow_html=True,
                )
            with col_txt:
                st.markdown(
                    f"<div style='background:#f0f4f8; border-left:4px solid #2980b9; "
                    f"border-radius:8px; padding:12px 16px; margin-bottom:10px;'>"
                    f"<b style='color:#1a5276; font-size:1.05rem;'>{titulo}</b><br>"
                    f"<span style='color:#34495e;'>{descripcion}</span></div>",
                    unsafe_allow_html=True,
                )

    st.markdown(
        "<p style='text-align:center; color:#aab7b8; font-size:0.85rem; margin-top:20px;'>"
        "📈🌊 Mareas del Canal de Panamá · HIMH · EIDEMAR · Creador: JFRodriguez</p>",
        unsafe_allow_html=True,
    )
    st.stop()
...
rango = st.sidebar.date_input("Rango", value=(max(fmin, pd.Timestamp("2004-01-01").date()), fmax),
...
h1, h2, h3, h4 = st.columns([0.65, 0.75, 4.2, 1.2])
with h1:
    if os.path.exists(_logo_himh):
        st.image(_logo_himh, width=95)
with h2:
    if os.path.exists(_logo_eidemar):
        st.image(_logo_eidemar, width=95)
with h3:
    st.markdown(
        f"### 📈🌊 {NOMBRES.get(est_activa, est_activa)}\n"
        f"HIMH · EIDEMAR · {f_ini} → {f_fin} · **Creador: JFRodriguez**"
    )
with h4:
    st.markdown(
        f"<div style='background:{badge_color};color:white;padding:8px 12px;"
        f"border-radius:12px;text-align:center;margin-top:10px;font-size:0.88rem;'>"
        f"📐 Archivo en <b>{badge_txt}</b></div>",
        unsafe_allow_html=True,
    )
...
with tabs[9]:
    ...
    st.caption(
        "La tendencia puede evaluarse con promedios mensuales o anuales. "
        "En modo estándar usa el rango filtrado del tablero; en modo robusto usa toda la serie cargada, exige más datos, filtra atípicos y muestra verificación."
    )

    if "trend_robusta_activa" not in st.session_state:
        st.session_state["trend_robusta_activa"] = False

    if st.button("🛡️ Activar / desactivar cálculo robusto y verificación", key="btn_trend_robusta"):
        st.session_state["trend_robusta_activa"] = not st.session_state["trend_robusta_activa"]

    modo_robusto = st.session_state["trend_robusta_activa"]
    df_tend_src = df_all if modo_robusto else df
    fuente_tend = "Toda la serie cargada" if modo_robusto else "Rango filtrado actual"
    min_mensual = 24 if modo_robusto else 3
    min_anual = 5 if modo_robusto else 3
    filtrar_outliers = bool(modo_robusto)

    if modo_robusto:
        st.success(
            "Modo robusto activo: usa toda la serie disponible, exige ≥24 meses o ≥5 años, filtra atípicos agregados y muestra intervalo de confianza."
        )
    else:
        st.info("Modo estándar activo: usa el rango filtrado actual; filtrado MAD de atípicos siempre activo para consistencia con STL.")

    # FIX-3: filtrar_outliers siempre True para consistencia con STL (robust=True)
    tendencia_mensual = calcular_tendencia_nivel(
        df_tend_src, col, base="mensual", min_puntos=min_mensual, filtrar_outliers=True
    )
    tendencia_anual = calcular_tendencia_nivel(
        df_tend_src, col, base="anual", min_puntos=min_anual, filtrar_outliers=True
    )

    comp_rows = []
    if tendencia_mensual:
        comp_rows.append({
            "Base de cálculo": "Promedios mensuales",
            "Fuente": fuente_tend,
            "N agregados": int(tendencia_mensual["n_utilizado"]),
            "Atípicos excluidos": int(tendencia_mensual["n_outliers"]),
            f"Pendiente ({u}/año)": round(tendencia_mensual["slope"], 5),
            "Equivalente (mm/año)": round(tendencia_mensual["slope_mm_year"], 2),
            "IC95% inferior (mm/año)": round(tendencia_mensual["ci_low_mm_year"], 2) if pd.notna(tendencia_mensual["ci_low_mm_year"]) else np.nan,
            "IC95% superior (mm/año)": round(tendencia_mensual["ci_high_mm_year"], 2) if pd.notna(tendencia_mensual["ci_high_mm_year"]) else np.nan,
            "R²": round(tendencia_mensual["r2"], 3),
            "p-valor": round(tendencia_mensual["p_val"], 4),
        })
    if tendencia_anual:
        comp_rows.append({
            "Base de cálculo": "Promedios anuales",
            "Fuente": fuente_tend,
            "N agregados": int(tendencia_anual["n_utilizado"]),
            "Atípicos excluidos": int(tendencia_anual["n_outliers"]),
            f"Pendiente ({u}/año)": round(tendencia_anual["slope"], 5),
            "Equivalente (mm/año)": round(tendencia_anual["slope_mm_year"], 2),
            "IC95% inferior (mm/año)": round(tendencia_anual["ci_low_mm_year"], 2) if pd.notna(tendencia_anual["ci_low_mm_year"]) else np.nan,
            "IC95% superior (mm/año)": round(tendencia_anual["ci_high_mm_year"], 2) if pd.notna(tendencia_anual["ci_high_mm_year"]) else np.nan,
            "R²": round(tendencia_anual["r2"], 3),
            "p-valor": round(tendencia_anual["p_val"], 4),
        })
    if comp_rows:
    ...
    resultado = tendencia_anual if base_tend == "Promedios anuales" else tendencia_mensual
    ...
    if resultado:
        ...
        tm1.metric("Pendiente en serie elegida", f"{resultado['slope']:+.5f} {u}/año")
        tm2.metric("Equivalente", f"{resultado['slope_mm_year']:+.2f} mm/año")
        tm3.metric("IC95%", f"[{resultado['ci_low_mm_year']:+.2f}, {resultado['ci_high_mm_year']:+.2f}] mm/año")
        tm4.metric("R²", f"{resultado['r2']:.3f}")
        sig = "✅ Significativa" if resultado["p_val"] < 0.05 else "⚠️ No significativa"
        tm5.metric("p-valor", f"{resultado['p_val']:.4f} ({sig})")

        # FIX-4: ascenso total acumulado equivalente al STL endpoint diff
        n_años_tend = resultado["periodo_fin"] - resultado["periodo_ini"]
        ascenso_total_mm = resultado["slope_mm_year"] * n_años_tend
        ta1, ta2, ta3 = st.columns(3)
        ta1.metric(
            "Ascenso total acumulado (OLS)",
            f"{ascenso_total_mm:+.1f} mm",
            help=f"slope_mm_año × {n_años_tend} años · comparable al Δtendencia STL endpoint"
        )
        ta2.metric("Período de cálculo", f"{resultado['periodo_ini']}–{resultado['periodo_fin']}")
        ta3.metric(
            "IC95% ascenso total",
            f"[{resultado['ci_low_mm_year']*n_años_tend:+.1f}, {resultado['ci_high_mm_year']*n_años_tend:+.1f}] mm"
            if pd.notna(resultado["ci_low_mm_year"]) else "N/D"
        )

        if tendencia_mensual and tendencia_anual:
            mismo_signo = np.sign(tendencia_mensual["slope_mm_year"]) == np.sign(tendencia_anual["slope_mm_year"])
            consistencia = "✅ Consistentes" if mismo_signo else "⚠️ Revisar consistencia"
            st.markdown(
                f"**Comparación rápida:** mensual = {tendencia_mensual['slope_mm_year']:+.2f} mm/año · anual = {tendencia_anual['slope_mm_year']:+.2f} mm/año · {consistencia}."
            )
        ...
        if base_tend == "Promedios anuales":
            st.success(
                f"La base anual es la recomendable para redactar resultados del tipo {periodo_ini}–{periodo_fin}."
            )
        else:
            st.info(
                "La base mensual sirve para exploración; la anual suele ser más estable para redacción técnica."
            )

        fig_t = go.Figure()
        if modo_robusto and resultado.get("serie_original") is not None and len(resultado["serie_original"]) != len(resultado["serie"]):
            fig_t.add_trace(go.Scatter(
                x=resultado["serie_original"].index,
                y=resultado["serie_original"].values,
                mode="markers",
                name="Serie agregada original",
                marker=dict(size=5, color="rgba(127,140,141,0.55)"),
            ))
        fig_t.add_trace(go.Scatter(
            x=resultado["serie"].index,
            y=resultado["serie"].values,
            mode="lines+markers",
            name=resultado["etiqueta_serie"],
            line=dict(color=color["linea"], width=2.5),
            marker=dict(size=6),
        ))
        fig_t.add_trace(go.Scatter(
            x=resultado["serie"].index,
            y=resultado["trend_line"],
            mode="lines",
            name=f"Tendencia lineal ({resultado['slope_mm_year']:+.2f} mm/año)",
            line=dict(color="#e74c3c", dash="dash", width=2),
        ))
        fig_t.update_layout(
            xaxis_title="Fecha",
            yaxis_title=resultado["etiqueta_y"],
            template="plotly_white",
            height=500,
            hovermode="x unified",
            margin=dict(l=50, r=20, t=30, b=50),
        ...
        with st.expander("🔎 Verificación del cálculo", expanded=modo_robusto):
            ver1, ver2, ver3, ver4 = st.columns(4)
            ver1.metric("Fuente usada", fuente_tend)
            ver2.metric("N original", f"{resultado['n_total_agregado']}")
            ver3.metric("N usado", f"{resultado['n_utilizado']}")
            ver4.metric("Atípicos excluidos", f"{resultado['n_outliers']}")

            st.markdown(
                "**Lógica aplicada:** 1) agregar por mes o por año; 2) opcionalmente filtrar atípicos con MAD robusto; "
                "3) ajustar regresión lineal; 4) reportar pendiente, IC95%, R² y p-valor."
            )

            df_tend_out = pd.DataFrame({
                "Fecha": resultado["serie"].index,
                resultado["etiqueta_y"]: resultado["serie"].values,
                f"Tendencia ajustada ({u})": resultado["trend_line"],
            })
            st.dataframe(df_tend_out, use_container_width=True, hide_index=True)

        if base_tend == "Promedios anuales":
            st.markdown(
                f"**Redacción sugerida para artículo:** El ajuste lineal de los promedios anuales del nivel del mar en {sitio_redaccion} "
                f"para {periodo_ini}–{periodo_fin} arroja {resultado['slope_mm_year']:+.2f} mm/año "
                f"(IC95%: {resultado['ci_low_mm_year']:+.2f} a {resultado['ci_high_mm_year']:+.2f} mm/año)."
            )
        else:
            st.markdown(
                f"**Redacción sugerida para el app:** El ajuste lineal de los promedios mensuales del nivel del mar en {sitio_redaccion} "
                f"para {periodo_ini}–{periodo_fin} arroja {resultado['slope_mm_year']:+.2f} mm/año "
                f"(IC95%: {resultado['ci_low_mm_year']:+.2f} a {resultado['ci_high_mm_year']:+.2f} mm/año)."
            )
    else:
        st.info("No hay suficientes datos para calcular una tendencia robusta con la configuración actual.")
...
# ═══════════════════════════════════════════════════════════════
with tabs[10]:
    st.subheader("🔬 Descomposición STL del Nivel del Mar")
    st.caption(
        "Seasonal-Trend decomposition using Loess (STL) — Cleveland et al. 1990. "
        "Separa la serie en: tendencia (LOESS), componente estacional y residuo. "
        "La tasa OLS sobre la tendencia LOESS es directamente comparable con "
        "la regresión del tab Tendencia (consistencia metodológica)."
    )

    if not _STL_AVAILABLE:
        with st.spinner("Instalando statsmodels... (solo la primera vez)"):
            import subprocess, sys
            try:
                subprocess.check_call(
                    [sys.executable, "-m", "pip", "install", "statsmodels", "-q"],
                ...
                st.error(
                    f"No se pudo instalar statsmodels automáticamente: {_install_err}\n\n"
        ...
        with stl_col1:
            stl_period = st.number_input("Período estacional (meses)", 6, 24, 12, 1, key="stl_period")
            stl_seasonal_w = st.number_input("Ventana estacional (impar)", 7, 25, 13, 2, key="stl_seasonal")
        with stl_col2:
            stl_trend_w = st.number_input("Ventana tendencia (impar)", 13, 49, 25, 2, key="stl_trend")
            stl_robust_flag = st.checkbox("Robusto (ponderación LOESS)", value=True, key="stl_robust")
        with stl_col3:
            stl_unidad_sel = st.radio("Unidad STL", ["Metros (m)", "Pies (ft)"], key="stl_unidad_sel")
            stl_col_use = "nivel_m" if "Metros" in stl_unidad_sel else "nivel_ft"
            stl_u = "m" if "Metros" in stl_unidad_sel else "ft"
            stl_factor_mm = 1000.0 if stl_u == "m" else FT_TO_M * 1000.0

        # Construir serie mensual completa (FIX-2: resample MS)
        serie_stl = (
            df_all[["fecha", stl_col_use]]
            .dropna()
            .set_index("fecha")[stl_col_use]
            .resample("MS")
            .mean()
            .dropna()
        )

        min_meses_req = int(stl_period) * 2 + int(stl_trend_w)
        if len(serie_stl) < min_meses_req:
            st.warning(
                f"Se necesitan ≥{min_meses_req} meses. Disponibles: {len(serie_stl)}. "
                "Reduce las ventanas o carga más datos."
            )
        else:
            s_win = int(stl_seasonal_w) if int(stl_seasonal_w) % 2 == 1 else int(stl_seasonal_w) + 1
            t_win = int(stl_trend_w)    if int(stl_trend_w) % 2 == 1    else int(stl_trend_w) + 1
            try:
                stl_obj = _STL(
                    serie_stl,
                    ...
                    seasonal=s_win,
                    trend=t_win,
                    robust=stl_robust_flag,
                )
                stl_result = stl_obj.fit()
            except Exception as _e:
                st.error(f"Error STL: {_e}")
                stl_result = None

            if stl_result is not None:
                _obs   = stl_result.observed
                _trend = stl_result.trend
                _seas  = stl_result.seasonal
                _resid = stl_result.resid

                # FIX-3: OLS sobre componente de tendencia STL → tasa comparable
                from scipy.stats import linregress as _lr
                _stl_x = np.arange(len(_trend), dtype=float)
                _sl, _ic, _rv, _pv, _se = _lr(_stl_x, _trend.values)
                _n_años = (
                    _trend.index[-1].year + _trend.index[-1].month / 12.0
                    - _trend.index[0].year  - _trend.index[0].month / 12.0
                )
                _rate_mm_yr        = _sl * 12.0 * stl_factor_mm
                _total_endpoint_mm = (_trend.iloc[-1] - _trend.iloc[0]) * stl_factor_mm
                _total_ols_mm      = _rate_mm_yr * _n_años
                _rango_est_cm      = (_seas.max() - _seas.min()) * stl_factor_mm / 10.0
                _trend_ols_line    = _ic + _sl * _stl_x

                # ── Métricas ──
                st.markdown("#### Métricas de la descomposición STL")
                m1, m2, m3, m4, m5 = st.columns(5)
                m1.metric(
                    "Tasa OLS sobre tendencia STL",
                    f"{_rate_mm_yr:+.2f} mm/año",
                    help="Regresión lineal sobre la tendencia LOESS. "
                         "Comparable directamente con slope_mm_year del tab Tendencia."
                )
                m2.metric(
                    "Ascenso total (endpoint LOESS)",
                    f"{_total_endpoint_mm:+.1f} mm",
                    help="trend[-1] − trend[0] en mm. Captura aceleración real si la tendencia no es lineal."
                )
                m3.metric(
                    "Ascenso total (OLS sobre STL)",
                    f"{_total_ols_mm:+.1f} mm",
                    help="rate_mm_año × años del período. Comparable con el tab Tendencia."
                )
                m4.metric(
                    "Rango estacional",
                    f"{_rango_est_cm:.1f} cm",
                    help="Amplitud pico a pico de la componente estacional."
                )
                m5.metric(
                    "R² (OLS/tend. STL)",
                    f"{_rv**2:.4f}",
                    help="Bondad del ajuste lineal sobre la tendencia LOESS."
                )

                _dif = abs(_total_endpoint_mm - _total_ols_mm)
                if _dif > 5.0:
                    st.info(
                        f"ℹ️ Diferencia endpoint LOESS vs OLS sobre STL: **{_dif:.1f} mm** — "
                        "la tendencia presenta aceleración/desaceleración real. "
                        "Para el artículo usar la tasa OLS sobre STL para comparar con tab Tendencia."
                    )

                st.markdown("---")
                # ── Figura 4 paneles ──
                st.markdown("#### Descomposición completa (4 paneles)")
                ...
                _fig = _ms(
                    ...
                    subplot_titles=[
                        ...
                        "(c) Componente estacional (cm)",
                        "(d) Componente residual (cm)",
                    ],
                )
                _fig.add_trace(go.Scatter(
                    x=_obs.index, y=_obs.values,
                    mode="lines", name="Observada",
                    line=dict(color="#2c3e50", width=0.8)), row=1, col=1)

                _fig.add_trace(go.Scatter(
                    x=_trend.index, y=_trend.values,
                    mode="lines", name="Tendencia LOESS",
                    line=dict(color="#c0392b", width=2.0)), row=2, col=1)
                _fig.add_trace(go.Scatter(
                    x=_trend.index, y=_trend_ols_line,
                    mode="lines",
                    name=f"OLS tendencia ({_rate_mm_yr:+.2f} mm/año)",
                    line=dict(color="#e74c3c", width=1.5, dash="dash")), row=2, col=1)

                _seas_cm = _seas.values * stl_factor_mm / 10.0
                _fig.add_trace(go.Scatter(
                    x=_seas.index, y=_seas_cm,
                    mode="lines", name="Estacional",
                    line=dict(color="#2980b9", width=0.9)), row=3, col=1)
                _fig.add_hline(y=0, line_dash="dash", line_color="gray", line_width=0.4, row=3, col=1)

                _resid_cm = _resid.values * stl_factor_mm / 10.0
                _fig.add_trace(go.Scatter(
                    x=_resid.index, y=_resid_cm,
                    mode="lines", name="Residuo",
                    line=dict(color="#27ae60", width=0.7)), row=4, col=1)
                _fig.add_hline(y=0, line_dash="dash", line_color="gray", line_width=0.4, row=4, col=1)

                _fig.update_yaxes(title_text=f"Nivel ({stl_u})", row=1, col=1)
                _fig.update_yaxes(title_text=f"Tendencia ({stl_u})", row=2, col=1)
                _fig.update_yaxes(title_text="Estacional (cm)", row=3, col=1)
                _fig.update_yaxes(title_text="Residuo (cm)", row=4, col=1)
                _fig.update_xaxes(title_text="Fecha", row=4, col=1)
                _sitio_stl = NOMBRES.get(est_activa, est_activa)
                _fig.update_layout(
                    height=840, template="plotly_white", hovermode="x unified",
                    margin=dict(l=70, r=20, t=60, b=50),
                    title_text=(
                        f"Descomposición STL — {_sitio_stl} "
                        f"({serie_stl.index[0].strftime('%Y-%m')} a "
                        f"{serie_stl.index[-1].strftime('%Y-%m')})"
                    ),
                    title_font_size=13,
                )
                ...
                st.markdown("#### Patrón estacional promedio (climatología mensual STL)")
                _sdf = _seas.to_frame("s").assign(month=lambda d: d.index.month)
                _clim = _sdf.groupby("month")["s"].agg(["mean", "std"])
                _clim["mc"] = _clim["mean"] * stl_factor_mm / 10.0
                _clim["sc"] = _clim["std"]  * stl_factor_mm / 10.0
                _bcols = ["#3498db" if v >= 0 else "#e74c3c" for v in _clim["mc"]]
                _figc = go.Figure()
                _figc.add_trace(go.Bar(
                    x=MESES, y=_clim["mc"],
                    error_y=dict(type="data", array=_clim["sc"].tolist(), visible=True),
                    marker_color=_bcols, marker_line_color="white",
                    marker_line_width=0.5, opacity=0.85, name="Anomalía estacional",
                ))
                _figc.add_hline(y=0, line_dash="dash", line_color="gray", line_width=0.5)
                _figc.update_layout(
                    xaxis_title="Mes", yaxis_title="Anomalía estacional media (cm)",
                ...
                st.markdown("---")
                st.markdown("#### Comparación de métodos — coherencia para artículo")
                _cmp = []
                _ols_ref = calcular_tendencia_nivel(
                    df_all, stl_col_use, base="mensual", min_puntos=24, filtrar_outliers=True
                )
                if _ols_ref:
                    _nyols = _ols_ref["periodo_fin"] - _ols_ref["periodo_ini"]
                    _cmp.append({
                        "Método": "OLS promedios mensuales (tab Tendencia)",
                        "Tasa (mm/año)": f"{_ols_ref['slope_mm_year']:+.2f}",
                        "Ascenso total (mm)": f"{_ols_ref['slope_mm_year'] * _nyols:+.1f}",
                        "Período": f"{_ols_ref['periodo_ini']}–{_ols_ref['periodo_fin']}",
                        "R²": f"{_ols_ref['r2']:.4f}",
                        "p-valor": f"{_ols_ref['p_val']:.4f}",
                        "Nota": "Regresión lineal directa sobre datos"
                    })
                _cmp.append({
                    "Método": "OLS sobre tendencia LOESS (STL)",
                    "Tasa (mm/año)": f"{_rate_mm_yr:+.2f}",
                    "Ascenso total (mm)": f"{_total_ols_mm:+.1f}",
                    "Período": f"{_trend.index[0].year}–{_trend.index[-1].year}",
                    "R²": f"{_rv**2:.4f}",
                    "p-valor": f"{_pv:.4f}",
                    "Nota": "OLS sobre componente suavizado LOESS"
                })
                _cmp.append({
                    "Método": "Endpoint LOESS (trend[-1] − trend[0])",
                    "Tasa (mm/año)": f"{_total_endpoint_mm / _n_años:+.2f}" if _n_años > 0 else "—",
                    "Ascenso total (mm)": f"{_total_endpoint_mm:+.1f}",
                    "Período": f"{_trend.index[0].year}–{_trend.index[-1].year}",
                    "R²": "—",
                    "p-valor": "—",
                    "Nota": "No lineal; captura aceleración real"
                })
                st.dataframe(pd.DataFrame(_cmp), use_container_width=True, hide_index=True)

                st.markdown(
                    f"**Redacción sugerida:** La descomposición STL del nivel del mar en "
                    f"{_sitio_stl} "
                    f"({serie_stl.index[0].strftime('%Y-%m')} a "
                    f"{serie_stl.index[-1].strftime('%Y-%m')}) "
                    f"extrae una tendencia LOESS con tasa OLS de **{_rate_mm_yr:+.2f} mm/año**. "
                    f"El ascenso total estimado por los extremos de la tendencia es "
                    f"**{_total_endpoint_mm:+.1f} mm**. "
                    f"La componente estacional presenta un rango de **{_rango_est_cm:.1f} cm**."
                )

                # ── Exportar ──
                with st.expander("📋 Exportar descomposición STL"):
                    _dfexp = pd.DataFrame({
                        "Fecha": _obs.index,
                        f"Observada_{stl_u}": _obs.values,
                        f"Tendencia_LOESS_{stl_u}": _trend.values,
                        f"Tendencia_OLS_{stl_u}": _trend_ols_line,
                        "Estacional_cm": _seas_cm,
                        "Residuo_cm": _resid_cm,
                    })
                    st.download_button(
                        "⬇️ CSV descomposición STL",
                        _dfexp.to_csv(index=False).encode("utf-8"),
                        f"STL_{est_activa}_{serie_stl.index[0].strftime('%Y%m')}_"
                        f"{serie_stl.index[-1].strftime('%Y%m')}.csv",
                        "text/csv",
                    )
...
# ═══════════════════════════════════════════════════════════════
# TAB 14 — PASO BAJO PUENTES
# ═══════════════════════════════════════════════════════════════
with tabs[14]:
    st.subheader("🚢 Paso bajo puentes")
    st.caption(
        "Evalúa la luz vertical disponible bajo el puente usando la estación de marea correcta según el sector. "
        "El puente Atlántico se evalúa con LMB; los puentes del Pacífico con AMA o DHT."
    )
    ...
    with bc1:
        ...
        st.markdown(f"**Datum de referencia del puente:** {info_p['datum_ref']}")
        st.markdown(f"**Sector:** {info_p['lado']}")

        opciones_puente = [e for e in PUENTE_ESTACION_PREF.get(puente_sel, []) if e in datasets]
        if opciones_puente:
            puente_estacion = st.selectbox(
                "Estación de marea usada",
                opciones_puente,
                index=0,
                format_func=lambda x: NOMBRES.get(x, x),
                key="puente_estacion_sel"
            )
            df_bridge_src = datasets[puente_estacion].copy()
        else:
            puente_estacion = None
            df_bridge_src = None
            st.error(
                f"No se encontró la estación requerida para {puente_sel}. "
                f"Carga {', '.join(PUENTE_ESTACION_PREF.get(puente_sel, []))} para evaluar este puente."
            )
        ...
        if tipo_buque == "Personalizado":
            air_draft = st.number_input("Calado aéreo del buque (m)", value=55.0, step=0.1, key="ad_custom")
        else:
            air_draft = st.number_input("Calado aéreo del buque (m)", value=air_draft_default, step=0.1, key="ad_val")

        st.markdown("---")
        nivel_actual_mode = st.radio("Nivel de agua a usar", ["Último observado", "Manual"], key="niv_mode")

        if df_bridge_src is not None:
        ...
        if nivel_actual_mode == "Último observado" and df_bridge_src is not None:
            ...
            st.info(
                f"Nivel actual en {NOMBRES.get(puente_estacion, puente_estacion)}: **{nivel_marea_m:.3f} m** ({nivel_marea_ft:.2f} ft)"
            )
            st.caption(f"Hora del dato: {fecha_nivel}")
        else:
            valor_manual_default = float(df_bridge_src["nivel_m"].iloc[-1]) if df_bridge_src is not None and len(df_bridge_src) else 0.7
        ...
        if df_bridge_src is not None:
            ...
            st.markdown(
                ...
                f"- Para **{puente_sel}**, el cálculo usa únicamente estaciones del mismo litoral."
            )
            if margen >= 3.0:
                ...
                st.error(f"🚫 Margen: {margen:.2f} m — ALTURA INSUFICIENTE")
        else:
            luz_disponible = np.nan
            margen = np.nan

    with bc2:
        if df_bridge_src is None:
            st.info("Carga la estación correcta para visualizar la evolución temporal de la luz disponible bajo el puente.")
        else:
            st.markdown("#### Luz disponible en el tiempo")
            _df_bridge = df_bridge_src.copy()
            if info_p["datum_ref"] == "MHW":
                ref_series = datums_bridge_m.get("MHW", {}).get("valor", _df_bridge["nivel_m"].mean())
            else:
                ref_series = datums_bridge_m.get("MSL", {}).get("valor", _df_bridge["nivel_m"].mean())
            _df_bridge["delta"] = _df_bridge["nivel_m"] - ref_series
            _df_bridge["altura_libre_m"] = info_p["clearance_m"] - _df_bridge["delta"]

            ult_bridge = _df_bridge[_df_bridge["fecha"] >= _df_bridge["fecha"].max() - pd.Timedelta(days=7)]
            dp_b, _ = resam(ult_bridge, "altura_libre_m", limite=5000)

            fig_bridge = go.Figure()
            fig_bridge.add_trace(go.Scattergl(
                x=dp_b["fecha"], y=dp_b["altura_libre_m"],
                mode="lines", name="Luz disponible",
                line=dict(color="#2563EB", width=2),
                fill="tozeroy", fillcolor="rgba(37,99,235,0.08)"
            ))
            fig_bridge.add_hline(y=air_draft, line_dash="dash", line_color="#e74c3c",
                annotation_text=f"Calado aéreo del buque: {air_draft:.1f} m", line_width=2)
            ...
            bc_s1.metric("Luz mínima", f"{_df_bridge['altura_libre_m'].min():.2f} m")
            bc_s2.metric("Luz máxima", f"{_df_bridge['altura_libre_m'].max():.2f} m")
            bc_s3.metric("Luz promedio", f"{_df_bridge['altura_libre_m'].mean():.2f} m")
            horas_peligro = (_df_bridge["altura_libre_m"] < air_draft).sum()
...
st.markdown(
    ...
    "🌊 Mareas del Canal de Panamá · HIMH Sección de Hidrología<br>"
    "EIDEMAR Escuela Internacional de Doctorado de Estudios del Mar<br>"
    "Datos: Autoridad del Canal de Panamá (ACP) · Creador: JFRodriguez<br>"
    "<b>v2.2</b> — Fix: skiprows·resample·outliers · Ascenso total · STL integrado</div>",
    unsafe_allow_html=True,
)