...
_dir = _os.path.dirname(__file__) or "."
_logo_himh = _os.path.join(_dir, "LOGO_HIMH.jpg")
_logo_eidemar = _os.path.join(_dir, "LOGO_EIDEMAR.png")

# Logos en sidebar
_sb_cols = st.sidebar.columns(2)
if _os.path.exists(_logo_himh):
    _sb_cols[0].image(_logo_himh, width=100)
if _os.path.exists(_logo_eidemar):
    ...
    if archivos_locales:
        ...
        for arc in archivos_locales:
            ...
            if st.sidebar.checkbox(nombre_disp, value=True, key=f"chk_{arc}"):
                try:
                    datasets[est] = cargar_csv(arc)
                    st.sidebar.success(f"✅ {est}: {len(datasets[est]):,} reg.")
...
st.markdown(
    f"<div style='display:flex;align-items:center;'>{_logos}<div>"
    f"<h1 style='color:#1a5276;margin:0;'>🌊 {NOMBRES.get(est_activa, est_activa)}</h1>"
    f"<p style='color:#5d6d7e;margin:0;'>HIMH Sección de Hidrología · "
    f"EIDEMAR Escuela Internacional de Doctorado de Estudios del Mar<br>"
    f"{f_ini} → {f_fin} · <b>Creador: JFRodriguez</b></p></div></div>",
    unsafe_allow_html=True,
)
st.markdown("")
...
with tabs[10]:
    ...
    if len(anios_disp) > 1:
        ...
        if anios_sel:
            ...
            # Nivel promedio diario superpuesto por año
            ...
            # Tendencia MSL anual
            st.subheader("Tendencia del MSL anual")
...
st.markdown(
    ...
    "Datos: Autoridad del Canal de Panamá (ACP) · Creador: JFRodriguez</div>",
    unsafe_allow_html=True,
)
