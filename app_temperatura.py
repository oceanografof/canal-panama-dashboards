...
def load_temperature_file(path: str) -> pd.DataFrame:
    ...
    if p.suffix.lower() in {".xlsx", ".xls"}:
        ...
        try:
            ...
            df = pd.read_csv(path, sep=None, skiprows=1, engine="python")
...
def _parse_delimited_tide_text(path: str) -> pd.DataFrame:
    ...
    df = pd.DataFrame(best_rows, columns=["fecha", "nivel_ft"])
    return df
...
if not temp_sources_meta:
    searched = "\n".join([f"- {p}" for p in candidate_directories()])
    st.error("No se encontraron archivos fuente de temperatura dentro del proyecto. El app ahora busca CSV, TXT y Excel en la carpeta del script, /data, /fuentes y /mnt/data.")
    st.info(f"Rutas revisadas:\n{searched}")
    st.stop()
