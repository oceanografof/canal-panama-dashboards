...
def load_temperature_file(path: str, mtime: float = 0) -> pd.DataFrame:  # mtime invalida caché automáticamente
    p = Path(path)
...
def load_tide_file(path: str, mtime: float = 0) -> pd.DataFrame:  # mtime invalida caché automáticamente
    p = Path(path)
    if p.suffix.lower() in {".xlsx", ".xls"}:
        df = pd.read_excel(path)
        if df.shape[1] < 2:
...
def load_wind_file(path: str, mtime: float = 0) -> pd.DataFrame:  # mtime invalida caché automáticamente
    p = Path(path)
    if p.suffix.lower() in {".xlsx", ".xls"}:
        df = pd.read_excel(path, skiprows=1)
        if df.shape[1] < 3:
...
if st.sidebar.button(
    ...
    help="Limpia la caché y re-escanea todos los archivos. Úsalo cuando subas nuevos archivos al repo.",
):
    st.cache_data.clear()
    st.rerun()
...
for src in temp_sources_meta:
    try:
        _mtime = Path(src.path).stat().st_mtime if Path(src.path).exists() else 0.0
        df_temp = load_temperature_file(src.path, mtime=_mtime)
...
for src in wind_sources_meta:
    try:
        _mtime = Path(src.path).stat().st_mtime if Path(src.path).exists() else 0.0
        df_w = load_wind_file(src.path, mtime=_mtime)
        if not df_w.empty:
...
COUNTER_FILE = Path(__file__).parent / ".visit_counter.json"
...
if "counted" not in st.session_state:
    ...
    _counter_data = _load_counter()
    _counter_data["total"] += 1
    _now_local = datetime.utcnow() - timedelta(hours=5)  # UTC-5 Panamá
    _counter_data["last_visit"] = _now_local.strftime("%d %b %Y, %H:%M")
    _save_counter(_counter_data)
else:
    _counter_data = _load_counter()

_total_visits = _counter_data.get("total", 1)
_last_visit   = _counter_data.get("last_visit", "—")
...
st.markdown(
    ...
    <div style="display:flex; justify-content:center; gap:16px; margin-top:1rem; flex-wrap:wrap;">
        <div style="background:#f4f6f8; border:1px solid #e0e0e0; border-radius:12px; padding:10px 22px; text-align:center; min-width:130px;">
            <div style="font-size:11px; color:#888;">Visitas totales</div>
            <div style="font-size:22px; font-weight:700; color:#2c3e50;">{_total_visits:,}</div>
        </div>
        <div style="background:#f4f6f8; border:1px solid #e0e0e0; border-radius:12px; padding:10px 22px; text-align:center; min-width:180px;">
            <div style="font-size:11px; color:#888;">Última visita (hora Panamá)</div>
            <div style="font-size:13px; font-weight:600; color:#2c3e50; margin-top:4px;">{_last_visit}</div>
        </div>
        <div style="background:#f4f6f8; border:1px solid #e0e0e0; border-radius:12px; padding:10px 22px; text-align:center; min-width:130px;">
            <div style="font-size:11px; color:#888;">Sesión activa</div>
            <div style="font-size:13px; font-weight:600; color:#27ae60; margin-top:4px;">&#9679; En línea</div>
        </div>
    </div>
    """,
    unsafe_allow_html=True,
