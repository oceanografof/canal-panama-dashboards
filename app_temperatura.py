...
def discover_source_files() -> list[SourceFile]:
    ...
    for root in candidate_directories():
        for path in root.rglob("*"):
            ...
            if ext in [".xlsx", ".xls"]:
                if "WATER TEMP" in name or "TEMP" in name:
                    sensor = infer_temp_sensor(path.name)
                    station = infer_station_from_name(path.name)
                    found.append(SourceFile(str(path), "Temperatura", sensor, station, source_label("Temperatura", sensor, station, str(path))))
                elif "WIND SPEED" in name or "WS AVG" in name or "WIND" in name:
                    station = infer_station_from_name(path.name)
                    found.append(SourceFile(str(path), "Viento", "WS AVG", station, source_label("Viento", "WS AVG", station, str(path))))
            elif ext == ".csv":
                if "BULKEXPORT" in name or "LEVEL" in name or "TIDE" in name or "MAREA" in name:
                    station = infer_station_from_name(path.name)
                    found.append(SourceFile(str(path), "Marea", "Nivel", station, source_label("Marea", "Nivel", station, str(path))))
    uniq = {}
    for item in found:
...
def load_temperature_excel(path: str) -> pd.DataFrame:
    df = pd.read_excel(path, skiprows=1)
    df = df.iloc[:, :3].copy()
    df.columns = ["fecha_inicio", "fecha_fin", "temp_raw"]
    df["fecha"] = pd.to_datetime(df["fecha_inicio"], errors="coerce")
    df["temp_c"] = pd.to_numeric(df["temp_raw"].astype(str).str.replace(",", ".", regex=False), errors="coerce")
    df = df.dropna(subset=["fecha", "temp_c"])
    df = df[(df["temp_c"] >= 10) & (df["temp_c"] <= 45)].copy()
...
def load_tide_csv(path: str) -> pd.DataFrame:
    df = pd.read_csv(path, skiprows=4, names=["fecha", "nivel_ft"], low_memory=False)
...
def load_wind_excel(path: str) -> pd.DataFrame:
    df = pd.read_excel(path, skiprows=1)
    df = df.iloc[:, :3].copy()
    df.columns = ["fecha_inicio", "fecha_fin", "viento_raw"]
    df["fecha"] = pd.to_datetime(df["fecha_inicio"], errors="coerce")
    df["viento_ms"] = pd.to_numeric(df["viento_raw"].astype(str).str.replace(",", ".", regex=False), errors="coerce")
    df = df.dropna(subset=["fecha", "viento_ms"])
    df = df[(df["viento_ms"] >= 0) & (df["viento_ms"] <= 60)].copy()
...
if not temp_sources_meta:
    st.error("No se encontraron archivos fuente de temperatura dentro del proyecto (carpeta del app, /data o /fuentes).")
    st.stop()
...
for src in temp_sources_meta:
    try:
        df_temp = load_temperature_excel(src.path)
...
for src in tide_sources_meta:
    try:
        df_tide = load_tide_csv(src.path)
...
if tide_frames:
    tide_keys = list(tide_frames.keys())
    pref_tide = "LMB" if "LMB" in tide_keys else tide_keys[0]
    selected_tide_key = st.sidebar.selectbox("Serie de marea integrada", tide_keys, index=tide_keys.index(pref_tide))
if wind_frames:
...
st.sidebar.caption("El app usa archivos que estén dentro del proyecto, en la carpeta del script, /data o /fuentes.")
...
st.markdown(
    ...
    <div style="display:flex; align-items:center; justify-content:space-between; gap:16px;">
        <div>
            <h1 style="margin:0; color:#c0392b;">🌡️ Dashboard Integrado de Temperatura, Marea y Viento</h1>
            <p style="margin:6px 0 0 0; color:#5d6d7e;">
...
if selected_wind_key:
    with tabs[tab_i]:
        ...
        st.caption("La serie de viento se toma automáticamente desde los archivos integrados dentro del proyecto.")
        ...
        temp_h = df.set_index("fecha")[[col_t]].resample("1h").mean()
        wind_h = df_viento.set_index("fecha")[[col_v]].resample("1h").mean()
        ...
        if mv.empty or len(mv) < 50:
            ...
            k2.metric("Correlación", f"{corr:.3f}")
            k3.metric("Viento medio", f"{mv[col_v].mean():.2f} {uv}")
            k4.metric("Viento máximo", f"{mv[col_v].max():.2f} {uv}")

            c1, c2 = st.columns(2)
            with c1:
                sample = mv.sample(min(6000, len(mv)), random_state=42)
                fig = go.Figure()
                fig.add_trace(go.Scattergl(x=sample[col_v], y=sample[col_t], mode="markers", marker=dict(size=4, opacity=0.32, color=sample[col_t], colorscale="RdYlBu_r", colorbar=dict(title=unidad))))
                ...
                st.plotly_chart(fig, use_container_width=True)
            with c2:
                s = mv.set_index("fecha").resample("6h").mean().dropna().reset_index()
                fig2 = make_subplots(specs=[[{"secondary_y": True}]])
                fig2.add_trace(go.Scatter(x=s["fecha"], y=s[col_t], mode="lines", line=dict(color=C["rojo"]), name="Temp"), secondary_y=False)
                fig2.add_trace(go.Scatter(x=s["fecha"], y=s[col_v], mode="lines", line=dict(color=C["azul"]), name="Viento"), secondary_y=True)
                fig2.update_yaxes(title_text=f"Temp ({unidad})", secondary_y=False)
                fig2.update_yaxes(title_text=f"Viento ({uv})", secondary_y=True)
                fig2.update_layout(template="plotly_white", height=400, hovermode="x unified", margin=dict(l=45, r=55, t=20, b=40))
                st.plotly_chart(fig2, use_container_width=True)

            lags = list(range(-48, 49, 2))
            vals, best = lag_correlation(mv, col_t, col_v, lags)
            fig3 = go.Figure()
            fig3.add_trace(go.Scatter(x=lags, y=vals, mode="lines+markers", line=dict(color=C["morado"], width=2)))
            fig3.add_vline(x=0, line_dash="dash", line_color=C["gris"])
...
st.markdown(
    ...
    <div style="text-align:center; color:#7f8c8d; font-size:0.90rem;">
        Dashboard Integrado de Temperatura, Marea, Viento y Surgencias · Canal de Panamá · HIMH · ACP<br>
