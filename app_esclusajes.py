...
def cargar_datos(fuente):
    ...
    # Crear columnas MCF si no existen
    hm3_to_mcf = {
    ...
    # Referencia operativa: suma de cámaras
    df["total_camaras"] = df[["numlockgat", "numlockpm", "numlockac", "numlockccl"]].sum(axis=1, min_count=1)
...
def preparar_mensual(df):
    out = df.copy()
    out["ym"] = out["fecha"].dt.to_period("M")
    mensual = out.groupby("ym", as_index=False).agg(
        gatlockhm3=("gatlockhm3", "sum"),
        pmlockhm3=("pmlockhm3", "sum"),
        aclockhm3=("aclockhm3", "sum"),
        ccllockhm3=("ccllockhm3", "sum"),
        total_hm3=("total_hm3", "sum"),
        pnx_transitos=("pnx_transitos", "mean"),
        npx_transitos=("npx_transitos", "mean"),
        total_transitos=("total_transitos", "mean"),
        total_camaras=("total_camaras", "mean"),
    )
...
k5.metric("Cámaras/día prom.", f"{df['total_camaras'].mean():.1f}")
k6.metric(periodo_nombre + "s", f"{df['periodo_label'].nunique()}")
st.caption("PNX y NPX se calculan por promedio entre las esclusas del complejo; la suma cruda de cámaras se conserva como referencia operativa.")
st.markdown("---")
...
with tabs[0]:
    ...
    for esc in esclusas:
        fig0.add_trace(
            go.Bar(
                x=mensual["fecha"],
                y=mensual[cols_esc[esc].replace("mcf", "hm3") if unidad == "hm³" else cols_esc[esc]],
                name=esc,
        ...
        filas.append(
            {
                ...
                "Cámaras/día prom.": round(df[cn].mean(), 2),
                f"Consumo/cámara ({uv})": round(df[cv].sum() / max(df[cn].sum(), 1), 3),
            }
        )
...
with tabs[1]:
    ...
    st.subheader("Tránsitos totales diarios")
    roll_t = df.set_index("fecha")[["total_transitos", "total_camaras"]].rolling("30D").mean().reset_index()
    fig1b = go.Figure()
    fig1b.add_trace(go.Scattergl(
        x=roll_t["fecha"], y=roll_t["total_transitos"], mode="lines",
        name="Tránsitos por complejo", line=dict(color=COL["Tránsitos"], width=3)
    ))
    fig1b.add_trace(go.Scattergl(
        x=roll_t["fecha"], y=roll_t["total_camaras"], mode="lines",
        name="Cámaras totales", line=dict(color="#95a5a6", width=2, dash="dot")
    ))
    fig1b.update_layout(
...
with tabs[3]:
    ...
    for i, per in enumerate(ordenar_periodos(df["periodo_label"].unique())):
        ...
        if usa_af:
            inicio = pd.to_datetime((sub["af"].iloc[0] - 1).astype(str) + "-10-01") if hasattr(sub["af"].iloc[0], "astype") else pd.Timestamp(year=int(sub["af"].iloc[0]) - 1, month=10, day=1)
            sub["dia_periodo"] = (sub["fecha"] - inicio).dt.days + 1
            xlabel = "Día del año fiscal"
        else:
            inicio = pd.to_datetime(sub["anio"].iloc[0].astype(str) + "-01-01") if hasattr(sub["anio"].iloc[0], "astype") else pd.Timestamp(year=int(sub["anio"].iloc[0]), month=1, day=1)
            sub["dia_periodo"] = (sub["fecha"] - inicio).dt.days + 1
            xlabel = "Día del año"
...
with tabs[4]:
    ...
    fig4b = go.Figure()
    if usa_af:
        for idx, mes_nom in enumerate(MESES_FISCALES):
            sub = df[df["mes_nombre"] == mes_nom]
            fig4b.add_trace(go.Box(y=sub[cols_vol["Total"]], name=mes_nom, boxmean=True))
        category_order = MESES_FISCALES
    else:
        for idx, mes_nom in enumerate(MESES_CAL):
            sub = df[df["mes_nombre"] == mes_nom]
            fig4b.add_trace(go.Box(y=sub[cols_vol["Total"]], name=mes_nom, boxmean=True))
        category_order = MESES_CAL
    fig4b.update_layout(
        template="plotly_white",
        height=400,
        showlegend=False,
...
with tabs[5]:
    ...
    with r2:
        ...
        st.metric("Cámaras prom/día", f"{df['total_camaras'].mean():.2f}")
...
with tabs[7]:
    ...
    st.caption("Para PNX y NPX la eficiencia se calcula usando tránsitos por complejo, no la suma cruda de cámaras.")
    ...
    with e1:
        ...
        fig7a.update_layout(
            ...
            yaxis_title=f"Consumo/cámara ({uv})", margin=dict(l=50, r=20, t=40, b=50)
        )
...
with tabs[8]:
    ...
    with r1:
        ...
        top_consumo = df[["fecha", cols_vol["Total"], "total_transitos", "total_camaras"]].sort_values(cols_vol["Total"], ascending=False).head(15).copy()
    ...
    with r2:
        ...
        top_trans = df[["fecha", "pnx_transitos", "npx_transitos", "total_transitos", "total_camaras"]].sort_values("total_transitos", ascending=False).head(15).copy()
...
with tabs[9]:
    ...
    if len(mensual_t) >= 3:
        ...
        future_dates = pd.date_range(
            mensual_t["fecha"].min(), periods=len(mensual_t) + horizonte, freq="MS"
        )
        fig9 = go.Figure()
...
with tabs[10]:
    ...
    with ex2:
        ...
        mens = df.assign(ym=df["fecha"].dt.to_period("M")).groupby("ym", as_index=False).agg(
            **{f"{esc}_prom": (cols_esc[esc], "mean") for esc in esclusas},
            ...
            camaras_prom=("total_camaras", "mean"),
        )
    ...
    mostrar = df[[
        ...
        cols_vol["Total"], "pnx_transitos", "npx_transitos", "total_transitos", "total_camaras"
    ]].copy()
