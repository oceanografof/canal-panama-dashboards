...
# ============================================================
# AUTO-ACTUALIZACIÓN — Aquatic Informatics BulkExport
# ============================================================
AUTO_UPDATE_URL = (
...
FULL_HISTORY_URL = AUTO_UPDATE_URL.replace(
    "DateRange=Custom&Period=P90D", "DateRange=EntirePeriodOfRecord"
)

# Metadatos — MISMO ORDEN que Datasets[0..3] en la URL de arriba
DATASET_META = [
    {"name": "LAN_WT_AVG_AMA",      "variable": "Temperatura", "sensor": "LAN",         "station": "AMA"},
    {"name": "Telemetria_TEMP_AMA",  "variable": "Temperatura", "sensor": "Telemetría",  "station": "AMA"},
    {"name": "WS_AVG_LMB",          "variable": "Viento",      "sensor": "WS AVG",      "station": "LMB"},
    {"name": "LAN_WS_AVG_FLC",      "variable": "Viento",      "sensor": "LAN WS AVG",  "station": "FLC"},
]
# Tiempo de caché en segundos (1 hora = datos horarios del servidor)
AUTO_CACHE_TTL   = 3600
_AQ_TIMEOUT_CONN = 30    # segundos — subido de 20 a 30
_AQ_TIMEOUT_READ = 120
_AQ_CHUNK        = 65536
...
def discover_source_files() -> list[SourceFile]:
    ...
    # Excluir el directorio de caché de Aquatic Informatics — esos archivos
    # los inyecta fetch_aquatic_informatics_bulk() directamente en all_sources_map.
    # Si se descubren aquí también, los archivos viejos (sin normalizar) causan
    # errores de parseo cuando el servidor no está accesible.
    _aq_cache_dir = str((Path(tempfile.gettempdir()) / "aq_info_auto_update").resolve())
    for root in candidate_directories():
        for path in root.rglob("*"):
            if not path.is_file() or path.suffix.lower() not in valid_ext:
                continue
            if str(path.resolve()).startswith(_aq_cache_dir):
                continue   # saltar archivos del caché automático
...
def persist_uploaded_source_files(uploaded_files) -> list[SourceFile]:
    ...
    return sorted(uniq.values(), key=lambda x: (x.variable, x.station, x.sensor, x.path))
# ============================================================
# DESCARGA AUTOMÁTICA — Aquatic Informatics
# ============================================================

def _aq_normalize_section(lines: list[str]) -> str:
    """
    Convierte una sección del BulkExport de Aquatic Informatics al formato
    de 3 columnas que espera el resto del dashboard:
        fecha_inicio,fecha_fin,valor_raw

    El BulkExport puede tener:
    - Líneas de metadata/comentario  →  # Key: Value  (se descartan)
    - 2 timestamps + valor           →  "StartTS","EndTS","Value","Grade",...
    - 1 timestamp  + valor           →  "Timestamp","Value","Grade",...

    En el caso de 1 solo timestamp, fecha_fin = fecha_inicio + 1 h (proxy).
    """
    # 1. Filtrar comentarios y líneas vacías
    data_lines = [l for l in lines
                  if l.strip() and not l.strip().startswith("#")]
    if not data_lines:
        return ""

    # 2. Detectar separador mirando el header
    ...
    header_parts = [h.lower() for h in split_row(header_raw)]

    # 3. Localizar columnas de timestamp y valor
    TS_KEYWORDS  = ("time", "stamp", "fecha", "iso", "utc", "start", "inicio")
    VAL_KEYWORDS = ("value", "valor", "°c", "degc", "m/s", "mph", "ft",
                    "temp", "wind", "nivel", "speed")

    ts_cols  = [i for i, h in enumerate(header_parts)
                if any(k in h for k in TS_KEYWORDS)]
    val_cols = [i for i, h in enumerate(header_parts)
                if any(k in h for k in VAL_KEYWORDS)]

    # Fallback posicional si las keywords no matchean
    if not ts_cols:
        ts_cols = [0]
    if not val_cols:
        # Si hay 2+ timestamps, el valor va en col 2; si hay 1, en col 1
        val_cols = [2 if len(ts_cols) >= 2 else 1]

    ts1_idx = ts_cols[0]
    ts2_idx = ts_cols[1] if len(ts_cols) >= 2 else None
    val_idx  = val_cols[0]

    # 4. Construir CSV normalizado
    ...
    for line in data_lines[1:]:
        ...
        # Saltar filas cortas o con valor ausente
        if n <= max(ts1_idx, val_idx):
            continue
        val_str = parts[val_idx] if val_idx < n else ""
        if not val_str or val_str.lower() in ("", "nan", "null", "none", "--"):
            continue

        # Intentar convertir valor a número (descartar filas de encabezado repetido)
        try:
            float(val_str.replace(",", ".").replace(" ", ""))
        except ValueError:
            continue

        ts1 = parts[ts1_idx] if ts1_idx < n else ""
        if not ts1:
            continue

        # ts2: usar la segunda columna de timestamp si existe; si no, repetir ts1
        if ts2_idx is not None and ts2_idx < n and parts[ts2_idx].strip():
            ts2 = parts[ts2_idx]
        else:
            ts2 = ts1   # fecha_fin = fecha_inicio (el loader lo maneja igual)

        out.append(f"{ts1},{ts2},{val_str}")

    return "\n".join(out)


def _aq_download_text(url: str) -> tuple[str | None, str | None]:
    """
    Descarga la URL por streaming para evitar timeouts en archivos grandes.
    Retorna (texto, None) si OK  ó  (None, mensaje_error) si falla.
    """
    ...
    try:
        ...
        with urllib.request.urlopen(req, timeout=_AQ_TIMEOUT_CONN) as resp:
            chunks: list[bytes] = []
            deadline = time.time() + _AQ_TIMEOUT_READ
            while True:
                if time.time() > deadline:
                    return None, f"Timeout de lectura ({_AQ_TIMEOUT_READ}s)."
                chunk = resp.read(_AQ_CHUNK)
                if not chunk:
                    break
                chunks.append(chunk)
        ...
        return None, f"Timeout de conexión ({_AQ_TIMEOUT_CONN}s)."
    ...
    for enc in ["utf-8-sig", "utf-8", "latin-1", "cp1252"]:
        try:
            ...
            continue
    return None, "No se pudo decodificar la respuesta del servidor."


def _aq_split_sections(text: str) -> list[list[str]]:
    """Parte el BulkExport en secciones separadas por líneas en blanco."""
...
def _aq_sections_to_files(sections: list[list[str]], subdir: str) -> list[dict]:
    """
    Normaliza cada sección y la guarda como CSV limpio de 3 columnas.
    Siempre borra archivos anteriores antes de escribir para evitar que datos
    viejos (sin normalizar) queden en disco si la descarga falla la próxima vez.
    Retorna lista de dicts serializables (para pasar por @st.cache_data).
    """
    out_dir = Path(tempfile.gettempdir()) / subdir
    out_dir.mkdir(parents=True, exist_ok=True)

    # Limpiar archivos viejos del mismo directorio antes de escribir los nuevos
    ...
    result: list[dict] = []
    for section, meta in zip(sections, DATASET_META[: len(sections)]):
        normalized = _aq_normalize_section(section)
        if not normalized or normalized.count("\n") < 1:
            continue
        target = out_dir / (meta["name"] + ".csv")
        try:
...
# ── Función CACHEADA: retorna list[dict] (serializable por st.cache_data) ─
@st.cache_data(
    ttl=AUTO_CACHE_TTL,
    show_spinner="🌐 Descargando datos desde Aquatic Informatics…",
)
def _fetch_aquatic_raw(url: str = AUTO_UPDATE_URL) -> list[dict]:
    """
    DEBE retornar solo tipos básicos de Python.
    Nunca retornar dataclasses → UnserializableReturnValueError.
    """
    text, err = _aq_download_text(url)
    if err:
        # Limpiar archivos viejos para que no se carguen datos obsoletos
        _stale_dir = Path(tempfile.gettempdir()) / "aq_info_auto_update"
        if _stale_dir.exists():
            for f in _stale_dir.glob("*.csv"):
                try:
                    f.unlink()
                except Exception:
                    pass
        return [{"error": err}]
    sections = _aq_split_sections(text)
    rows = _aq_sections_to_files(sections, "aq_info_auto_update")
    return rows if rows else [{"error": "El servidor respondió pero no se encontraron datos válidos en el CSV."}]


# ── Wrapper NO cacheado: convierte dicts → SourceFile ─────────────────────
def fetch_aquatic_informatics_bulk(url: str = AUTO_UPDATE_URL) -> list[SourceFile]:
    raw = _fetch_aquatic_raw(url)
    if raw and "error" in raw[0]:
        st.sidebar.warning(f"⚠️ Aquatic Informatics: {raw[0]['error']}")
        return []
    return [
        SourceFile(
            path=d["path"],
            ...
            sensor=d["sensor"],
            station=d["station"],
            label=source_label(d["variable"], d["sensor"], d["station"], d["path"]),
        )
        for d in raw
    ]
...
def load_temperature_file(path: str) -> pd.DataFrame:
    ...
    if p.suffix.lower() in {".xlsx", ".xls"}:
        ...
        # ── PRIORIDAD: formato normalizado de download_data.py ──────────────
        # Header = "fecha_inicio,fecha_fin,valor_raw" en la primera fila.
        # NO usar skiprows=1 aquí — el header es la fila 0.
        try:
            raw = pd.read_csv(path, sep=",", engine="python")
            if list(raw.columns[:3]) == ["fecha_inicio", "fecha_fin", "valor_raw"] and raw.shape[0] > 0:
                df = raw.iloc[:, :3].copy()
        except Exception as exc:
            last_error = exc

        # ── FALLBACK: formatos legacy con metadata en fila 0 (skiprows=1) ───
        if df is None:
            for kwargs in [
...
# ── Descarga automática desde Aquatic Informatics ─────────────────────────
st.sidebar.markdown("### 🌐 Actualización automática")
_auto_enabled = st.sidebar.toggle(
    "Descargar datos desde Aquatic Informatics",
    value=True,
    help="Descarga los últimos 90 días cada hora. Requiere acceso al servidor desde Streamlit Cloud.",
)
_col_r1, _col_r2 = st.sidebar.columns(2)
if _col_r1.button("🔄 Actualizar\n(90 días)", help="Limpia caché y descarga los últimos 90 días."):
    _fetch_aquatic_raw.clear()
    st.rerun()
if _col_r2.button("📥 Histórico\ncompleto", help="Descarga el periodo entero (puede tardar minutos)."):
    _fetch_aquatic_raw.clear()
    st.session_state["_aq_use_full_history"] = True
    st.rerun()

_use_full   = st.session_state.get("_aq_use_full_history", False)
_active_url = FULL_HISTORY_URL if _use_full else AUTO_UPDATE_URL
_label      = "periodo completo" if _use_full else "últimos 90 días"

auto_fetch_sources: list[SourceFile] = []
if _auto_enabled:
    auto_fetch_sources = fetch_aquatic_informatics_bulk(url=_active_url)
    if auto_fetch_sources:
        st.sidebar.success(
            f"✅ {len(auto_fetch_sources)} dataset(s) · {_label} "
            f"(caché {AUTO_CACHE_TTL // 60} min)"
        )
        if _use_full:
            st.session_state["_aq_use_full_history"] = False
    else:
        # Detectar si fue timeout (indica bloqueo de red/VPN)
        _raw_check = _fetch_aquatic_raw.func if hasattr(_fetch_aquatic_raw, "func") else None
        st.sidebar.error(
            "🔒 **Sin acceso al servidor Aquatic Informatics**\n\n"
            "Streamlit Cloud (servidores en EEUU) no puede llegar a "
            "`panama.aquaticinformatics.net` porque está en la intranet ACP.\n\n"
            "**Opciones:**\n"
            "- Suba los CSV manualmente con el cargador de abajo\n"
            "- Use GitHub Actions para descargar y guardar los archivos en el repo "
            "(ver instrucciones en el expander de abajo)"
        )
        with st.sidebar.expander("📋 Cómo configurar descarga automática por GitHub Actions"):
            st.markdown(
                """
**Archivo `.github/workflows/update_data.yml`** en tu repositorio:

```yaml
name: Actualizar datos ACP
on:
  schedule:
    - cron: '0 * * * *'   # cada hora
...
jobs:
  download:
    ...
    steps:
      ...
      - name: Descargar CSV desde Aquatic Informatics
        run: |
          curl -o data/Telemetria_TEMP_AMA.csv \\
            "https://panama.aquaticinformatics.net/Export/BulkExport?..."
      - name: Commit y push
        run: |
          ...
          git add data/
          git diff --staged --quiet || git commit -m "Auto: datos $(date +%Y-%m-%d)"
          git push
```
> ⚠️ Esto solo funciona si los servidores de GitHub Actions tienen acceso a la red ACP. Si no, la única opción es subir los archivos manualmente.
                """
            )

st.sidebar.markdown("---")
st.sidebar.caption("Respaldo: suba archivos locales si la conexión falla.")
uploaded_runtime_files = st.sidebar.file_uploader(
    "Subir archivos fuente manualmente",
...
# Agregar datos descargados automáticamente (prioridad alta)
for item in auto_fetch_sources:
...
tide_frames: dict[str, pd.DataFrame] = {}
for src in tide_sources_meta:
    try:
        df_tide = load_tide_file(src.path)
        if not df_tide.empty:
            key = src.station if src.station not in tide_frames else f"{src.station} ({Path(src.path).stem})"
            tide_frames[key] = df_tide
    except Exception as exc:
        st.warning(f"No se pudo cargar marea {Path(src.path).name}: {exc}")
...
st.sidebar.markdown(f"**Marea:** {len(tide_frames)} serie(s)")
...
with st.sidebar.expander("Ver rutas detectadas"):
    ...
    if selected_tide_key:
        st.write(f"Marea activa: {selected_tide_key}")
...
# ── Indicador de último dato disponible ───────────────────────────────────
import datetime as _dt

st.sidebar.markdown("---")
st.sidebar.markdown("### 🕐 Último dato disponible")

_now_pan = _dt.datetime.utcnow() - _dt.timedelta(hours=5)  # UTC-5 Panamá

def _fmt_last(ts: pd.Timestamp, label: str) -> None:
    """Muestra fecha/hora del último dato con semáforo de frescura."""
    if pd.isna(ts):
        st.sidebar.caption(f"{label}: sin datos")
        return
    lag_h = max(0, (_now_pan - ts.replace(tzinfo=None)).total_seconds() / 3600)
    ...
    st.sidebar.markdown(
        ...
        f"{ts.strftime('%Y-%m-%d  %H:%M')}</span>  \n"
        f"<span style='font-size:0.77rem;color:#7f8c8d;'>{lag_str}</span>",
        unsafe_allow_html=True,
    )

# Temperatura — fuente activa seleccionada
_fmt_last(df_temp_main["fecha"].max(), f"Temp · {temp_key}")

# Sensores de temperatura adicionales (Telemetría y LAN por separado)
for _k, _df_k in temp_frames.items():
    if _k in (temp_key, "Combinado"):
        continue
    _fmt_last(_df_k["fecha"].max(), f"Temp · {_k}")

# Viento — todas las series disponibles
for _wk, _df_w in wind_frames.items():
    _fmt_last(_df_w["fecha"].max(), f"Viento · {_wk}")

if not tide_frames:
    st.sidebar.info("No se detectó archivo de marea LMB/AMA. El app mostrará las pestañas automáticamente cuando el archivo exista dentro del proyecto.")
if not wind_frames:
...
with tabs[tab_i]:
    ...
    with r2:
        ...
        if selected_tide_key:
            bullets.append(f"Serie de marea integrada: **{selected_tide_key}**.")
        if selected_wind_key:
...
if selected_tide_key:
    with tabs[tab_i]:
        st.subheader(f"Temperatura vs marea · {selected_tide_key}")
        df_marea = tide_frames[selected_tide_key].copy()
        temp_h = df.set_index("fecha")[[col_t]].resample("1h").mean()
        tide_h = df_marea.set_index("fecha")[["nivel_ft"]].resample("1h").mean()
        mt = temp_h.join(tide_h, how="inner").dropna().reset_index()

        if mt.empty or len(mt) < 50:
            st.warning("No hay suficiente traslape entre temperatura y marea en el rango seleccionado.")
        else:
            corr = mt[col_t].corr(mt["nivel_ft"])
            k1, k2, k3 = st.columns(3)
            k1.metric("Coincidencias", f"{len(mt):,}")
            k2.metric("Correlación", f"{corr:.3f}")
            k3.metric("Marea media", f"{mt['nivel_ft'].mean():.2f} ft")

            c1, c2 = st.columns(2)
            with c1:
                sample = mt.sample(min(6000, len(mt)), random_state=42)
                fig = go.Figure()
                fig.add_trace(go.Scattergl(x=sample["nivel_ft"], y=sample[col_t], mode="markers", marker=dict(color=C["rojo"], size=4, opacity=0.28)))
                fig.update_layout(template="plotly_white", height=380, xaxis_title="Nivel (ft)", yaxis_title=f"Temperatura ({unidad})", title=f"Dispersión · r={corr:.3f}", margin=dict(l=45, r=20, t=40, b=40))
                st.plotly_chart(fig, use_container_width=True)
            with c2:
                s = mt.set_index("fecha").resample("6h").mean().dropna().reset_index()
                fig2 = make_subplots(specs=[[{"secondary_y": True}]])
                fig2.add_trace(go.Scatter(x=s["fecha"], y=s[col_t], mode="lines", line=dict(color=C["rojo"]), name="Temp"), secondary_y=False)
                fig2.add_trace(go.Scatter(x=s["fecha"], y=s["nivel_ft"], mode="lines", line=dict(color=C["azul"]), name="Marea"), secondary_y=True)
                fig2.update_yaxes(title_text=f"Temp ({unidad})", secondary_y=False)
                fig2.update_yaxes(title_text="Nivel (ft)", secondary_y=True)
                fig2.update_layout(template="plotly_white", height=380, hovermode="x unified", margin=dict(l=45, r=55, t=20, b=40))
                st.plotly_chart(fig2, use_container_width=True)

            lags = list(range(-48, 49, 2))
            vals, best = lag_correlation(mt, col_t, "nivel_ft", lags)
            fig3 = go.Figure()
            ...
            fig3.add_vline(x=best, line_dash="dot", line_color=C["rojo"], annotation_text=f"Mejor desfase {best} h")
            fig3.update_layout(template="plotly_white", height=320, xaxis_title="Desfase (horas)", yaxis_title="Correlación", margin=dict(l=45, r=20, t=20, b=40))
...
# TAB — FORZANTES
# ============================================================
if selected_tide_key and selected_wind_key:
    with tabs[tab_i]:
        st.subheader(f"Forzantes integrados · Marea {selected_tide_key} y Viento {selected_wind_key}")
        df_marea = tide_frames[selected_tide_key].copy()
        df_viento = wind_frames[selected_wind_key].copy()
        tf = to_hourly(df, [col_t])
        mf = to_hourly(df_marea, ["nivel_ft"])
        wf = to_hourly(df_viento, ["viento_ms"])
        combo = tf.merge(mf, on="fecha", how="inner").merge(wf, on="fecha", how="inner")

        if combo.empty or len(combo) < 50:
            st.warning("No hay suficiente traslape entre temperatura, marea y viento para esta vista conjunta.")
        else:
            fig = make_subplots(specs=[[{"secondary_y": True}]])
            fig.add_trace(go.Scatter(x=combo["fecha"], y=combo[col_t], mode="lines", line=dict(color=C["rojo"]), name="Temperatura"), secondary_y=False)
            fig.add_trace(go.Scatter(x=combo["fecha"], y=combo["nivel_ft"], mode="lines", line=dict(color=C["azul"]), name="Marea"), secondary_y=True)
            fig.update_yaxes(title_text=f"Temperatura ({unidad})", secondary_y=False)
            fig.update_yaxes(title_text="Marea (ft)", secondary_y=True)
            fig.update_layout(template="plotly_white", height=420, hovermode="x unified", margin=dict(l=45, r=55, t=20, b=40))
            st.plotly_chart(fig, use_container_width=True)

            f1, f2 = st.columns(2)
            with f1:
                fig2 = go.Figure()
                fig2.add_trace(go.Scattergl(x=combo["viento_ms"], y=combo[col_t], mode="markers", marker=dict(size=4, opacity=0.25, color=combo["nivel_ft"], colorscale="Viridis", colorbar=dict(title="Marea ft"))))
                fig2.update_layout(template="plotly_white", height=360, xaxis_title="Viento (m/s)", yaxis_title=f"Temperatura ({unidad})", margin=dict(l=45, r=20, t=20, b=40))
                st.plotly_chart(fig2, use_container_width=True)
            with f2:
                stats_table = pd.DataFrame({
                    "Relación": ["Temp vs marea", "Temp vs viento", "Marea vs viento"],
                    "Correlación": [
                        round(combo[col_t].corr(combo["nivel_ft"]), 3),
                        round(combo[col_t].corr(combo["viento_ms"]), 3),
                        round(combo["nivel_ft"].corr(combo["viento_ms"]), 3),
                    ],
                })
                st.dataframe(stats_table, use_container_width=True, hide_index=True)
                st.markdown(
                    "\n".join([
                        "- Esta vista concentra en una sola pestaña la temperatura, la marea y el viento.",
                        "- Da más contexto al análisis de surgencias y ayuda a interpretar si el enfriamiento coincide con cambios en el forzamiento.",
                        f"- El crédito del tablero se mantiene como **{AUTHOR_FOOTER}**.",
                    ])
                )
    tab_i += 1
